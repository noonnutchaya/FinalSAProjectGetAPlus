...
class WorkDonePage extends Component {
  ...
  componentDidMount(){
    db.collection(dataCollectionName).where("stateWork", "==", "Done").onSnapshot(querySnapshot => {
        let userDataList = [];
    ...
    return (
      <div>
        ...
        <div className="layout">
          <h1 className="title"> งานที่เสร็จแล้ว </h1>
          <div>
              <table id='students'>
                  {this.renderTableData()}
              </table>
          </div>
          <h5 className="tip"> Tip: กดปุ่ม "Done" เพื่อเปลี่ยนสถานะงานเป็น "Received" ที่แสดงถึงรายการงานนี้ได้ส่งถึงมือลูกค้าเป็นที่เรียบร้อยแล้ว </h5>
        </div>
      </div>
    );
  }
